#!/usr/bin/env python3
"""
Simple WebSocket Bridge Service for Anvil Sim
...
async def main():
    ...
    logger.info(f"🔌 Starting WebSocket bridge on port {bridge.port}")
    ...
    async with websockets.serve(bridge.register_client, "0.0.0.0", bridge.port):
        logger.info(f"✅ WebSocket bridge running on port {bridge.port}")
